...
- Mattermost v11.7.9 (the version the current patch targets)
- Mattermost v11.6.6 also supported via [`patches/mattermost-v11.6.6.patch`](patches/mattermost-v11.6.6.patch)
- Mattermost v11.5.7 also supported via [`patches/mattermost-v11.5.7.patch`](patches/mattermost-v11.5.7.patch)
- Mattermost v11.4.5 also supported via [`patches/mattermost-v11.4.5.patch`](patches/mattermost-v11.4.5.patch)
...
> **Older patches need an older checkout**: Mattermost v11.6 changed the
> `einterfaces.OAuthProvider` interface (`GetUserFromJSON` gained a
> `settings *model.SSOSettings` parameter). The module at HEAD implements the new
> interface, so the v11.5.7-and-older patches must be used with this repository
> checked out at the commit that introduced them.
...
```bash
git clone --depth 1 --branch v11.7.9 https://github.com/mattermost/mattermost.git ../mattermost
```
...
```bash
cd ../mattermost && git apply ../mattermost-oidc/patches/mattermost-v11.7.9.patch
```
